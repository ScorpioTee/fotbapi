...
class FacrParserService
{
    ...
    {
        $html =$this->initDomDocument(sprintf(self::TABLE_URL_PATTERN, $uuid));
        $this->fillCompetitionTableData($uuid, $html);
    }

    private function fillCompetitionTableData(string $uuid, string $html): void
    {
        $crawler = new Crawler($html);

        $tableData = $crawler->filter('table tr')->each(function (Crawler $node, $i) use ($uuid) {
            if ($i === 0) return null;
            if ($i > 14) return null;
            ...
            if ($cells->count() > 0) {
                $scoreArray = explode(':', $cells->eq(6)->text());
                return [
                    'position' => trim($cells->eq(0)->text()),
                    'club'    => trim($cells->eq(1)->text()),
                    'win' => trim($cells->eq(3)->text()),
                    'draw'   => trim($cells->eq(4)->text()),
                    'lost'   => trim($cells->eq(5)->text()),
                    'goalsScored'   => trim($scoreArray[0]),
                    'goalsReceived'   => trim($scoreArray[1]),
                    'points'   => trim($cells->eq(7)->text()),
                ];
            }
            return null;
        });

        // Vyčistíme null hodnoty (z hlavičky)
        $tableData = array_values(array_filter($tableData));
        $this->data[$uuid] = $tableData;
    }
    ...
    private function initDomDocument($url): string
    {
